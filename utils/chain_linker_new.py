...
def link_mediators_in_payload(
    ...
    for res in analysis_results:
        ...
        # In the Visualizer/Database, this is often stored as a separate interaction row.
        ...
        if link_data:
            # We add this to extra_interactions_to_save for the database
            ...
            # The visualizer draws links based on 'interactions' list in snapshot.
            # We need to make sure this link ends up there.
            # Current pipeline architecture:
            # - runner.py saves 'extra_interactions_to_save' to DB.
            # - But what about the current JSON payload?
            # - The visualizer reads 'snapshot_json'.

            # We should inject this link into the main payload so it appears in the immediate output/visualizer.
            # However, the payload structure is usually a list of interactors relative to MAIN.
            # 'link_data' is relative to MEDIATOR.

            # HACK/SOLUTION: We can add a "detached" interaction if the visualizer supports it?
            # Or we simply rely on the 'upstream_interactor' field for the INDIRECT node,
            # and let the visualizer draw the path if it knows the nodes.
            # BUT, the visualizer specifically draws lines based on the interactions list.
            # If we want a physical line between Mediator and Indirect, we might need to add it to the snapshot interactions.

            # For now, we return these as 'extra' to be saved to DB.
            # If we want them in the current JSON, we might need a specific place.
            # But standard `ctx_json['interactors']` implies `main` <-> `primary`.

            # Let's rely on `upstream_interactor` update below for the logic,
            # and `extra_interactions_to_save` for the persistent DB edge.
            pass
